...
Elm.Core.make = function (_elm) {
   ...
   var genChanges = A2($Signal._op["<~"],
   function (_) {
      return _.x;
   },
   $Keyboard.arrows);
   var generations = A3($Signal.foldp,
   F2(function (x,n) {
      return $Basics.max(0)(n + x);
   }),
   0,
   genChanges);
   var evolve = F2(function (rules,
   state) {
      return $List.concat($List.map(function (s) {
         return $Maybe.withDefault(_L.fromArray([s]))(A2($Dict.get,
         s,
         rules));
      })(state));
   });
   var evolveTo = F2(function (_v0,
   gen) {
      return function () {
         return function () {
            var loop = F2(function (n,
            state) {
               return _U.cmp(n,
               gen) > -1 ? state : loop(n + 1)(A2(evolve,
               _v0.rules,
               state));
            });
            return A2(loop,0,_v0.axiom);
         }();
      }();
   });
   var states = function (lSys) {
      ...
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 $Basics.toFloat(_v0._0)) > 0 || _U.cmp(canvasHeight,
                 $Basics.toFloat(_v0._1)) > 0 ? A2($Basics.min,
                 $Basics.toFloat(_v0._0) / canvasWidth,
                 $Basics.toFloat(_v0._1) / canvasHeight) : 1.0;
                 var _ = $Debug.watch("scaleFactor")(scaleFactor);
                 var _ = $Debug.watch("canvas_area")(canvasArea);
                 var $ = $Debug.watch("canvas_area_centre")($Path.canvasCentre(canvasArea)),
                 centreX = $._0,
                 centreY = $._1;
                 var content = $Graphics$Collage.move({ctor: "_Tuple2"
                                                      ,_0: (0 - centreX) * scaleFactor
                                                      ,_1: (0 - centreY) * scaleFactor})($Graphics$Collage.scale(scaleFactor)($Graphics$Collage.group(segs)));
...
Elm.Path.make = function (_elm) {
   ...
   var canvasCentre = function (_v8) {
      return function () {
         return function () {
            var centreY = ($Basics.snd(_v8.topL) + $Basics.snd(_v8.bottomL)) / 2;
            var centreX = ($Basics.fst(_v8.topL) + $Basics.fst(_v8.topR)) / 2;
            return {ctor: "_Tuple2"
                   ,_0: centreX
                   ,_1: centreY};
         }();
   ...
   var calcEndPos = F3(function (_v14,
   rotation,
   length) {
      return function () {
         switch (_v14.ctor)
         {case "_Tuple2":
            return function () {
                 var endY = _v14._1 + length * $Basics.sin(rotation);
                 var endX = _v14._0 + length * $Basics.cos(rotation);
                 return {ctor: "_Tuple2"
                        ,_0: endX
                        ,_1: endY};
              }();}
